...
All subsequent commits go to this branch. Agent worktrees merge into it. When validated, use `gh pr create` to open PR to `base_branch` (see complete-milestone).
</step>
...
2. **Spawn executor agents:**
   ...
   **If worktree integration is active** (worktree_setup step ran), each agent gets a `<worktree_context>` block directing it to work in its assigned worktree directory.
   ...
   Task(
     ...
     prompt="
       ...
       - [ ] If worktree: all commands run from worktree directory
       </success_criteria>
     "
   )
   ```
...
4. **Report completion — spot-check claims first:**
   ...
   **If worktree integration active:** After spot-checks pass, merge and cleanup each agent:

   ```bash
   # Merge agent branch into base branch (current or phase branch)
   git merge --no-ff "$AGENT_BRANCH" -m "Merge $AGENT_BRANCH: {plan_name}"

   # If merge conflict: orchestrator resolves manually, stages, completes merge
   # After merge: delete agent branch and remove worktree
   git branch -d "$AGENT_BRANCH"
   git worktree remove "wt/agent-${AGENT_ID}"
   ```

   **Test execution:** Check config `test_after_merge`. If true, run test suite after each merge; fail if exit non-zero.
