...
**0. Plan type check (before any execution):** Read plan frontmatter. If `type: tdd`, follow `<tdd_execution>` section exclusively—do NOT iterate over tasks. TDD plans use RED-GREEN-REFACTOR cycle. Commit pattern: test(...), feat(...), refactor(...)—exactly 2-3 commits per TDD plan.
...
<tdd_execution>
When executing task with `tdd="true"`, or when plan frontmatter has `type: tdd` (canonical signal for plan-level TDD), follow RED-GREEN-REFACTOR cycle:
